...
freqs = input("Enter the filter passband frequencies: ")/(fs/2);
freqss = input("Enter the filter stopband frequencies: ")/(fs/2);
[N, wc] = buttord(freqs, freqss, specs(1), specs(2));
[b, a] = butter(N, wc, "stop");
freqz(b, a);
L = input("Enter the length of the signal:");
t = (0:L-1).*(1/fs);
s = 2.45*cos(2*pi*50*t) + 2*randn(size(t)) + -sqrt(2)*sin(2*pi*120*t);
figure(2);
subplot(2, 2, 1);
stem(1000*t, s);
