...
freqs = input("Enter the passband and stopband frequencies")/(fs/2);
[n, wc] = cheb1ord(freqs(1), freqs(2), specs(1), specs(2));
[b, a] = cheby1(n, specs(1), wc, "high");
freqz(b, a);
L = input("Enter the length of the signal: ");
t = (0:L-1).*(1/fs);
s = 2.45*cos(2*pi*120*t) + 2*randn(size(t));
y_new = filter(b, a, s);
figure(2);
